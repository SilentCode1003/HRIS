    ...
    try {
      let sql = `    
        ...
        FROM 
          master_employee
        WHERE 
          me_jobstatus IN ('regular', 'probationary')`;
...
router.get('/countterminated', (req, res,) => {
  try {
    let sql = `    
    SELECT 
    COUNT(*) AS Terminate
...
router.get('/countendofcontract', (req, res,) => {
  try {
    let sql = `    
    SELECT 
    COUNT(*) AS Endo
